      ...
      <ul class="nav">
        <template v-for="(item, index) in navItems">
          <template v-if="item.title">
            <CSidebarNavTitle :key="index"
                                 :name="item.name"
                                 :classes="item.class"
                                 :wrapper="item.wrapper"/>
          </template>
          <template v-else-if="item.divider">
            <CSidebarNavDivider :key="index"
                                :classes="item.class"/>
          </template>
          <template v-else-if="item.label">
            <CSidebarNavLabel  :key="index"
                                  :name="item.name"
                                  :url="item.url"
                                  :icon="item.icon"
                                  :label="item.label"
          ...
          <template v-else>
            <template v-if="item.children">
              ...
              :icon="item.icon">
                <template v-for="(childL1, index1) in item.children">
                  <template v-if="childL1.children">
                    ...
                    <CSidebarNavDropdown :key="index1"
                      ...
                      <li :key="index2"
                        ...
                        <CSidebarNavLink :name="childL2.name"
                                         :url="childL2.url"
                                         :icon="childL2.icon"
                                         :badge="childL2.badge"
                  ...
                  <template v-else>
                    <CSidebarNavItem :key="index1"
                                     :classes="item.class">
                      <CSidebarNavLink :name="childL1.name"
                                       :url="childL1.url"
                                       :icon="childL1.icon"
                                       :badge="childL1.badge"
                                       :variant="item.variant"/>
            ...
            <template v-else>
              <CSidebarNavItem :key="index"
                               :classes="item.class">
                <CSidebarNavLink :name="item.name"
                                 :url="item.url"
                                 :icon="item.icon"
                                 :badge="item.badge"
                                 :variant="item.variant"/>
